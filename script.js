...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function animateMultiStageFlight(points, callback) {
        ...
        const fly = document.createElement('div');
        fly.className = 'cyber-fly fixed-flight'; // Uses the new CSS class
        ...
        setTimeout(() => {
            ...
            if (remainingPoints.length > 1) {
                // If this point is intermediate (not final destination), maybe pause/hover?
                // Check if we are at a "Friend" location? We can just pass a flag or infer.
                // For now, smooth continue.

                // --- PAUSE AT FRIEND LOCATION ---
                // We create a temporary buzzer at the current location to simulate "Visiting"
                const pauseFly = document.createElement('div');
                pauseFly.className = 'cyber-fly fixed-flight buzzing'; // Buzzing state
                pauseFly.style.left = `${end.x}px`;
                pauseFly.style.top = `${end.y}px`;
                pauseFly.style.transform = `translate(-50%, -50%) rotate(${angle}deg)`;
                document.body.appendChild(pauseFly);
    ...
    function initTaskbarInsectBehavior(taskbarItem) {
        ...
        function behaviorLoop() {
            if (!document.body.contains(taskbarItem)) return; // Stop if removed (Restored)

            const action = Math.random();
            // 0.0 - 0.5: Sit & maybe blink
            // 0.5 - 0.8: Roam (Small movements inside/outside)
            // 0.8 - 1.0: "Talk" (Buzz)

            if (action < 0.6) { // Increased Sit probability
                // SIT (Strictly Sit & Blink inside)
                fly.classList.remove('buzzing');
                fly.style.transform = 'translate(-50%, -50%)'; // Reset to center

                // Blink Logic (Toggle Opacity)
                if (Math.random() < 0.4) {
                    fly.style.opacity = '0.4';
                    setTimeout(() => fly.style.opacity = '', 150);
                }

                setTimeout(behaviorLoop, 1000 + Math.random() * 2000);
            } else if (action < 0.85) { // Roam chance check
                // ROAM (Go outside border!) - RE-PARENTING STRATEGY
                // User requirement: "DONT CLONE IT... ICON BORDER INSIDE MUST BE EMPTY"

                const rect = fly.getBoundingClientRect();

                // 1. Promote to Body (Fixed)
                // We physically move the element so it leaves the taskbar container empty.
                fly.classList.add('fixed-flight');
                fly.style.position = 'fixed';
                fly.style.left = rect.left + 'px';
                fly.style.top = rect.top + 'px';
                fly.style.width = '6px';
                fly.style.height = '10px';
                fly.style.zIndex = '99999';
                fly.style.margin = '0';
                fly.style.transform = 'translate(0, 0) rotate(0deg)'; // Reset transform

                document.body.appendChild(fly); // MOVE it out

                // 2. Pick Target (Range +/- 80px)
                const range = 80;
                const endX = rect.left + (Math.random() - 0.5) * range * 2;
                const endY = rect.top + (Math.random() - 0.5) * range * 2 - 40; // Tend upwards

                // 3. Animate Out
                // Use a slight delay to allow DOM update
                requestAnimationFrame(() => {
                    fly.style.transition = 'all 1s ease-in-out';
                    fly.style.left = endX + 'px';
                    fly.style.top = endY + 'px';
                    const angle = Math.atan2(endY - rect.top, endX - rect.left) * (180 / Math.PI) + 90;
                    fly.style.transform = `rotate(${angle}deg)`;
                });

                // 4. Return after 2s
                setTimeout(() => {
                    // Check if taskbar item still exists (User might have closed it)
                    if (!document.body.contains(taskbarItem)) {
                        fly.remove(); // Cleanup if homeless
                        return;
                    }
                    ...
                    fly.style.transform = 'rotate(0deg)'; // Orient up/flat for landing

                    setTimeout(() => {
                        // 5. Re-Parent to Taskbar logic
                        if (document.body.contains(taskbarItem)) {
                            taskbarItem.appendChild(fly);

                            // Reset Styles to be "Inside"
                            fly.classList.remove('fixed-flight');
                            fly.style.position = '';
                            fly.style.width = '';
                            fly.style.height = '';
                            fly.style.zIndex = '';
                            ...
                            fly.style.transform = 'translate(-50%, -50%)'; // Original CSS center
                ...
                const blinkInterval = setInterval(() => {
                    fly.style.opacity = (fly.style.opacity === '0.3' ? '1' : '0.3');
                    blinkCount++;
                    if (blinkCount > 5) {
    ...
    const speakText = (text) => {
        if ('speechSynthesis' in window) {
            ...
            // Prefer a "Tech" voice (Google US English or similar)
            const preferredVoice = voices.find(v => v.name.includes('Google US English')) || voices[0];
            if (preferredVoice) utterance.voice = preferredVoice;
            utterance.pitch = 1.3; // Robotic/Bug-like
            utterance.rate = 1.1;
    ...
    const handleAIResponse = (userQuery) => {
        ...
        // 1. OFFLINE CHECK
        if (!navigator.onLine) {
            const offlineMsg = "🔴 <b>OFFLINE MODE:</b> CANNOT REACH HIVE MIND SERVER.";
            addInsectMessage(offlineMsg, 'ai');
            speakText("Offline mode. Cannot reach hive mind.");
            return;
        }

        // 2. SERVER CONNECT SIMULATION
        ...
        setTimeout(() => {
            ...
            if (query.includes('who are you') || query.includes('what is this')) {
                responseHTML = "I AM <b>BUZZ AI</b> 🐝<br>A VIRTUAL ASSISTANT FOR THIS PORTFOLIO.<br>I CAN ANALYZE FILES, READ RESUMES, AND FETCH CONTACT DATA.";
                textToSpeak = "I am Buzz AI. A virtual assistant for this portfolio. I can analyze files, read resumes, and fetch contact data.";
            }
            else if (query.includes('contact') || query.includes('email') || query.includes('linkedin') || query.includes('hire') || query.includes('reach')) {
                responseHTML = `
                    ...
                    🔗 <a href="https://www.linkedin.com/in/mathews-b-designer" target="_blank" style="color:#00ff00; text-decoration:underline;">LinkedIn Profile</a><br>
                    🌐 <a href="https://www.mathewsb.in" target="_blank" style="color:#00ff00; text-decoration:underline;">www.mathewsb.in</a>
                `;
                textToSpeak = "Contact data found. You can email hello at mathewsb dot in, or visit the LinkedIn profile.";
            }
            else if (query.includes('google') || query.includes('search') || query.includes('internet') || query.includes('web')) {
                // Fake Web Search
                responseHTML = `
                    <b>🔍 SEARCH RESULTS FOR: "${userQuery}"</b><br><br>
                    I have accessed the global database.<br>
                    • Result 1: <u>${userQuery} - Wikipedia</u><br>
                    • Result 2: <u>Official Documentation</u><br><br>
                    <i>(Simulated Search Result)</i>
                `;
                textToSpeak = `I have accessed the global database for ${userQuery}. Displaying simulated results.`;
            }
            else if (query.includes('hello') || query.includes('hi') || query.includes('hey')) {
                responseHTML = "GREETINGS! SYSTEM ONLINE. ASK ME ABOUT MATHEWS B OR UPLOAD A FILE.";
                textToSpeak = "Greetings! System online. Ask me about Mathews B or upload a file.";
            }
            else {
                // Fallback "AI" answer
                responseHTML = `
                    <b>🤖 AI ANALYSIS:</b><br>
                    I checked the internal database for <i>"${userQuery}"</i>.<br>
                    My neural net suggests this is related to: <b>General Knowledge</b>.<br><br>
                    I am optimized to answer questions about <b>Mathews B's Portfolio</b>. Try asking "Who is Mathews?" or "Contact Info".
                `;
                textToSpeak = "I checked the internal database. I am optimized to answer questions about Mathews B's Portfolio.";
            }
    ...
    const handleFileUploadMock = (file) => {
        ...
        setTimeout(() => {
            ...
            if (file.type === 'application/pdf') {
                analysis = `<b>📄 PDF ANALYSIS COMPLETE</b><br>SIZE: ${(file.size / 1024).toFixed(1)} KB<br>STATUS: SAFE<br><br>SUMMARY: This document contains structured data. I can extract text layers and metadata.`;
                speech = "PDF Analysis Complete. Status Safe.";
            } else if (file.type.startsWith('image/')) {
                analysis = `<b>🖼️ IMAGE RECOGNITION</b><br>DIMENSIONS: DETECTED<br>CONTENT: VISUAL DATA<br><br>I see high-contrast pixels. It might be a design asset or screenshot.`;
                speech = "Image recognition complete. Visual data detected.";
            } else {
                analysis = "⚠️ FILE TYPE UNKNOWN. UNABLE TO DEEP SCAN.";
    ...
    if (insectFileInput) {
        insectFileInput.addEventListener('change', (e) => {
            if (e.target.files.length > 0) {
                handleFileUploadMock(e.target.files[0]);
                e.target.value = '';
            }
        });
    }
        ...
        const renderContent = () => {
            ...
            blogContentArea.innerHTML = `
                ...
                <div class="blog-nav-footer">
                    ...
                    <button class="nav-article-btn share-btn" onclick="shareArticle('${post.title.replace(/'/g, "\\'")}', window.location.href)">SHARE</button>
