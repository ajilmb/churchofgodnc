...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderAbout() {
        ...
        `;
        // Scroll Logic for About (Global Indicator)
        setTimeout(() => {
            const scrollWrapper = document.querySelector('.about-content .scroll-wrapper');
            ...
            if (scrollWrapper && indicator) {
                ...
                if (scrollWrapper.scrollHeight > scrollWrapper.clientHeight + 10) {
                    // Ensure Indicator is visible initially
                    indicator.classList.remove('hidden-indicator');
                    indicator.style.display = 'flex';

                    // Attach listener to this specific wrapper
                    const onScroll = () => {
                        if (scrollWrapper.scrollTop > 20) {
                            indicator.classList.add('hidden-indicator');
                        }
                    };
                    scrollWrapper.addEventListener('scroll', onScroll);
                    // Store for cleanup if needed, though simple page switch cleanup might be enough if we hide indicator on switch.
                } else {
                    indicator.classList.add('hidden-indicator');
                }
            }
    ...
    function renderBlog(filterText = '') {
        ...
        // Scroll Logic for Blog (Global Indicator)
        setTimeout(() => {
            ...
            // 1. Try finding whichever has overflow
            if (dynamicContainer && dynamicContainer.scrollHeight > dynamicContainer.clientHeight) {
                targetScroll = dynamicContainer;
            } else if (blogContent && blogContent.scrollHeight > blogContent.clientHeight) {
                targetScroll = blogContent;
            } else if (document.body.scrollHeight > window.innerHeight) {
                targetScroll = window;
            }

            if (indicator && targetScroll) {
                // Check Height Difference
                let scrollH, clientH;
                if (targetScroll === window) {
                    scrollH = document.body.scrollHeight;
                    clientH = window.innerHeight;
                } else {
                    scrollH = targetScroll.scrollHeight;
                    clientH = targetScroll.clientHeight;
                }

                if (scrollH > clientH + 50) {
                    indicator.classList.remove('hidden-indicator');
                    indicator.style.display = 'flex';

                    const onScroll = () => {
                        const scrollTop = targetScroll === window ? window.scrollY : targetScroll.scrollTop;
                        if (scrollTop > 20) {
                            indicator.classList.add('hidden-indicator');
                        }
                    };
                    targetScroll.addEventListener('scroll', onScroll);
                } else {
                    indicator.classList.add('hidden-indicator');
                }
            } else {
    ...
    window.updateContent = function () {
        ...
        // If on WORKS page, start the loop
        if (page.id === 'works' && typeof WORK_STATUS_DATA !== 'undefined') {
            window.workStatusInterval = setInterval(() => {
                toggleWorkHeader();
            }, WORK_STATUS_DATA.interval);
        } else {
            // Reset to default style if needed (remove specific classes)
            const iconContainer = document.getElementById('iconContainer');
            if (iconContainer) {
                iconContainer.classList.remove('work-status-active');
                // Ensure Icon is visible and Text is hidden or reset
                // (The lines above already reset innerHTML/textContent to default)
            }
        }
        // -----------------------------
